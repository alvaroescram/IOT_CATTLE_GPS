 ...
 **************************************************************/
 /*
  * X0M056M752EQ7RI3 MQTT API
  * VO63PBXFGWR2XI7E write API
  * 1022535 CHANNEL ID
  * 
  * 
  * DTH11, gy-906, GPS setup
  */
...
boolean publishAll(){
 ...
 float mlxTemp = mlx.readObjectTempC();
  String data2Send = "field1="+COWID+"&field2="+String(latitude,6)+"&field3="+String(longitude,6)+"&field6="+String(dhtTemp,2)+"&field7="+String(mlxTemp,2)+"&status=MQTTPUBLISH";
  SerialMon.println(dataReady);

  
//  return mqtt.publish(topicPing, "field1=100&field2=1&field3=10&field4=100&field5=1000&status=MQTTPUBLISH");
   return mqtt.publish(topicPing, dataReady);
}
...
void loop() {
  ...
  }else
  {
    ...
    if (!isMQTTConnected) 
    {
      ...
      if((millis()-lastReconnectedAttempt >= 10000L))//add is gps valid
      {
